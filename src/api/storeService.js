...
export async function getAllStores() {
    try {
        return await request('/api/stores/all', { method: 'GET' });
    } catch (err) {
        if (err.status === 404) {
            console.warn('Stores endpoint not found, returning empty data');
            return { data: [] };
        }
        throw err;
    }
...
export async function uploadStoreImage(storeId, file) {
    const token = getToken();
    const url = buildUrl(API_URL, `/api/stores/${encodeURIComponent(storeId)}/upload-image`);
